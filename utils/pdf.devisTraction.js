...
export function buildDevisTractionPDF(devis) {
  ...
  // Type de compte (on accepte plusieurs variantes)
  ...
  const accountType =
    ["societe", "société", "company", "enterprise", "entreprise"].includes(accountTypeRaw) ? "societe" :
    ["personnel", "personal", "particulier"].includes(accountTypeRaw) ? "personnel" : "";
  ...
  const perso = {
    ...
    poste: get(user, ["posteActuel", "personal.poste", "personnel.poste", "fonction", "role"]),
  };

  // Société (variantes tolérées)
  const soc = {
    nom:       get(user, ["nomSociete", "societe", "company", "company.name", "entreprise", "societyName"]),
    matricule: get(user, [
                 "matriculeFiscal","matricule_fiscal","company.matriculeFiscal",
                 "societe.matriculeFiscal","mf","MF","taxId","fiscalId"
               ]),
    poste:     get(user, ["company.poste", "societe.poste", "posteActuel", "fonction", "role"]),
  };
  ...
  // Numéro (format DDVYY#####: ex. DDV2500068) + Date
  ...
  doc
    ...
    .fillColor(C_MUTED)
    .text(
      `${formatDevisNumber(_id, createdAt, providedNumber)}  ·  ${createdAt ? dayjs(createdAt).format("DD/MM/YYYY HH:mm") : "-"}`,
      LEFT,
      titleY + 24,
  ...
  const infoItems = [
    { label: "Type de compte", value: accountType ? (accountType === "societe" ? "Société" : "Personnel") : "-" },
  ...
  if (accountType === "societe") {
    infoItems.push(
      { label: "Société",          value: pretty(soc.nom) },
    ...
    if (soc.nom)       infoItems.push({ label: "Société", value: pretty(soc.nom) });
...
function measureCellHeight(doc, { label = "", value = "" } = {}, width) {
  ...
  doc.x = save.x; doc.y = save.y; doc._font = save.font; doc._fontSize = save.size; doc._fillColor = save.fill;
  return hLabel + hValue + 8; // padding
}
...
function renderCell(doc, x, w, y, h, { label = "", value = "" } = {}, { C_TEXT, C_MUTED }) {
  ...
  doc
    ...
    .fontSize(9)
    .fillColor(C_MUTED)
    .text(label, x, y + pad, { width: w, align: "left" });
  ...
  doc.y = y + h; // verrouille la hauteur
}
...
function drawParagraph(doc, text, { LEFT, RIGHT, C_TEXT, placeholder = "-" }) {
  ...
  doc
    ...
    .fontSize(10.5)
    .fillColor(C_TEXT)
    .text(value, LEFT, doc.y, { width: RIGHT - LEFT, align: "left" });
}
...
/* ───────────────────── DATA HELPERS ───────────────────── */
// Retourne un numéro au format DDVYY##### (ex: DDV2500068)
function formatDevisNumber(id, createdAt, provided) {
  // 1) Si un numéro est fourni par la DB, on le respecte et on normalise
  if (provided) {
    const s = String(provided);
    if (/^DDV\d{7}$/.test(s)) return s; // déjà correct
    const digits = s.replace(/\D/g, "");
    if (digits.length >= 7) return `DDV${digits.slice(-7)}`; // garde 7 derniers chiffres
    const yy = dayjs(createdAt || Date.now()).format("YY");
    const serial = digits.padStart(5, "0").slice(-5);
    return `DDV${yy}${serial}`;
  }

  // 2) Sinon, dériver depuis l'id + date (stable)
  const yy = dayjs(createdAt || Date.now()).format("YY");
